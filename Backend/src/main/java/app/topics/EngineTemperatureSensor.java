...
import static app.AppConfigs.ENGINE_TEMP_HEAT;
import static app.AppConfigs.ENGINE_TEMP_OVERHEAT;
...
public class EngineTemperatureSensor implements Topic {
    ...
    public IMqttMessageListener getListener() {
        ...
        return (topic,msg) -> {
            ...
            if (engineTemperature >= ENGINE_TEMP_HEAT && !engineHeat) {
                LOGGER.info("Engine temperature has reached over {} degrees, AC system can function", ENGINE_TEMP_HEAT);
                engineHeat = true;
            }
            if (engineTemperature >= ENGINE_TEMP_OVERHEAT && !engineOverHeat && EngineStatusSensor.getEngineRunning()) {
                LOGGER.info("Engine temperature has reached over {} degrees, engine will stop in {} seconds", ENGINE_TEMP_OVERHEAT, 6 - engineOverHeatCount * 2);
                engineOverHeatCount++;
            }
            ...
            if (engineTemperature < ENGINE_TEMP_OVERHEAT) {
            ...
            if (engineTemperature < ENGINE_TEMP_HEAT) {
                engineHeat = false;
            }
        };
    }
